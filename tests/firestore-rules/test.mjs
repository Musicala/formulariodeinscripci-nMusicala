...
try {
  ...
  await test("staff puede leer integration_jobs escrito por Admin SDK", async () => {
    ...
    const db = testEnv.authenticatedContext("admin", { email: "imusicala@gmail.com" }).firestore();
  ...
  await test("registration_events no admite lectura pública ni de staff", async () => {
    ...
    const staffDb = testEnv.authenticatedContext("admin", { email: "imusicala@gmail.com" }).firestore();
  ...
  await test("el comodín administrativo conserva acceso a otras colecciones", async () => {
    const db = testEnv.authenticatedContext("admin", { email: "imusicala@gmail.com" }).firestore();
    await assertSucceeds(setDoc(doc(db, "catalogos", "general"), { active: true }));
    await assertSucceeds(getDoc(doc(db, "catalogos", "general")));
  });
